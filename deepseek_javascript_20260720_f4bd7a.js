...
// ============================================================
// VARIABLES GLOBALES (AJOUTÉES pour les corrections)
// ============================================================
let stopTimeout = null;
let minSearchTimer = null;
...
function togglePause(){
  ...
  if(!paused){
    ...
    // CORRECTION : Utiliser masterVolume au lieu de 0.25
    if(gainNode)gainNode.gain.setTargetAtTime(masterVolume,audioCtx.currentTime,0.1);
    if(timerMin>0){
...
function startAudio(){
  ...
  // Utiliser masterVolume au lieu de 0.25
  gainNode.gain.linearRampToValueAtTime(masterVolume, audioCtx.currentTime+2);
...
// stopAudio — CORRECTION avec stopTimeout
...
// Recherche globale dans tous les minéraux — AVEC DEBOUNCE (300ms)
function filterMinGlobal(query){
  clearTimeout(minSearchTimer);
  minSearchTimer = setTimeout(function() {
...
// playNextInSequence — CORRECTION : réinitialiser seqEveilDiffs
function playNextInSequence(){
  // Réinitialiser les diff spéciaux si on change de type de séquence
  if (seqType !== 'eveil' && seqType !== 'eveil_chakras') {
    seqEveilDiffs = [];
  }
